...
void PlayingState::ResetSong()
{
   ...
   const static microseconds_t LeadIn = 1000000;
   const static microseconds_t LeadOut = 5000000;
...
void PlayingState::Draw(Renderer &renderer) const
{
   ...
   m_keyboard->Draw(renderer, key_tex, note_tex, Layout::ScreenMarginX, 0, m_notes, m_show_duration / 5,
      m_state.midi->GetSongPositionInMicroseconds(), m_state.track_properties,
      m_state.midi->BeatLines(), m_state.midi->BarLines());

   if (m_paused)
   {
      renderer.SetColor(0, 0, 0, int(160));
      renderer.DrawQuad(0, GetStateHeight() / 3, GetStateWidth(), 100);
      const Color c = Renderer::ToColor(255, 255, 255, int(0xFF));
      TextWriter title(GetStateWidth()/2, GetStateHeight()/3 + 25, renderer, true, 24);
      title << Text(m_state.song_title, c);
      TextWriter begin(GetStateWidth() / 2, GetStateHeight() / 3 + 70, renderer, true, 16);
      begin << Text(pause_text, c);
   ...
   {
      int combo_font_size = 25;

      int combo_x = GetStateWidth() / 2;
      int combo_y = GetStateHeight() - CalcKeyboardHeight() - 80 - (combo_font_size/2);
